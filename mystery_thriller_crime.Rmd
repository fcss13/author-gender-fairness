...
if (.Platform$OS.type == "unix") {
  options( boot.ncpus = strtoi( system("nproc", intern=TRUE) ) )
...
```{r loadIBCF}
ibcf_boot_results <- read.csv( sprintf("output/%s_cf_boot_results_IBCF_1000.csv", BOOK_GENRE) )
...
```{r loadUBCF}
ubcf_boot_results <- read.csv( sprintf("output/%s_cf_boot_results_UBCF_1000.csv", BOOK_GENRE) )
...
```{r loadPOP}
pop_boot_results <- read.csv( sprintf("output/%s_cf_boot_results_POP_1000.csv", BOOK_GENRE) )
